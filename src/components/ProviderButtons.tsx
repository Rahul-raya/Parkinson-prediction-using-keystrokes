...
export const ProviderButtons = () => {
  ...
  const handleProviderLogin = async (provider: 'google' | 'microsoft' | 'apple') => {
    try {
      ...
      let user;
      if (provider === 'google') {
        user = await authAdapter.loginWithGoogle();
      } else if (provider === 'microsoft') {
        user = await authAdapter.loginWithMicrosoft();
      } else {
        user = await authAdapter.loginWithApple();
      }
  ...
  return (
    <div className="space-y-3">
      <Button
        type="button"
        variant="outline"
        className="w-full glass-card hover:border-primary/50 transition-all"
